...
export const createListsSlice: KanbanStateCreator<ListsSlice> = (set) => ({
    createList: (boardId, list) => set((state) => withBoard(state, boardId, (board) => {
        const id = crypto.randomUUID();
        board.lists.push({ id, items: [], ...list })
    })),
    editList: (boardId, listIndex, list) => set((state) => withBoard(state, boardId, (board) => {
        board.lists[listIndex] = {
            ...board.lists[listIndex], ...list
        }
    })),
    removeList: (boardId, listIndex) => set((state) => withBoard(state, boardId, (board) => {
        board.lists.splice(listIndex, 1)
    })),
    moveList: (boardId, activeListIndex, overListIndex) => set((state) => withBoard(state, boardId, (board) => {
